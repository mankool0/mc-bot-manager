...
void BotManager::tryInitializeWorldAutoSaver(BotInstance* bot)
{
    ...
    if (bot->worldAutoSaver && bot->worldAutoSaverServerIp != bot->server) {
        ...
        if (!stillUsed) {
            m_sharedWorldSavers.remove(oldKey);
        }
    }
    ...
    if (it != m_sharedWorldSavers.end()) {
        ...
        const WorldSaveSettings& incoming = bot->worldSaveSettings;
        if (existing.saveBlockEntities != incoming.saveBlockEntities ||
            existing.saveEntities      != incoming.saveEntities      ||
            existing.saveItemEntities  != incoming.saveItemEntities  ||
    ...
    for (int i = 0; i < botInstances.size(); ++i) {
        if (botInstances[i]->name == name) {
            ...
            if (!serverKey.isEmpty()) {
                ...
                if (!stillUsed) {
                    m_sharedWorldSavers.remove(serverKey);
                }
            }
...
{
    instance().sendSwitchHotbarSlotImpl(botName, slot);
}

void BotManager::sendSwitchHotbarSlotImpl(const QString &botName, int slot)
{
    BotInstance *bot = getBotByNameImpl(botName);
    if (!bot) {
        LogManager::log(QString("[%1] Bot not found for switch hotbar slot").arg(botName),
                       LogManager::Error);
        return;
    }
                                 ...
                                 mankool::mcbot::protocol::BlockFaceGadget::BlockFace face, bool sneak)
{
    BotInstance *bot = getBotByNameImpl(botName);
    if (!bot) {
        LogManager::log(QString("[%1] Bot not found for look_at").arg(botName),
                       LogManager::Error);
        return;
    }
