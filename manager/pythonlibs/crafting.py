...
def _plan_fits_in_2x2(plan, bot_name=""):
    ...
    for step in plan.get('steps', []):
        ...
        recipe = world.get_recipe(recipe_id, bot_name=bot_name)
...
def _open_crafting_container(bot_name="", max_distance=128, use_player_inventory=False):
    ...
    container = world.get_container(bot_name=bot_name)
    ...
    if use_player_inventory:
        ...
        crafting_table = world.find_nearest(["minecraft:crafting_table"],
                                            max_distance=max_distance, bot_name=bot_name)
        ...
        already_in_reach = world.can_reach_block(x, y, z, bot_name=bot_name)
        if not already_in_reach:
            # Find nearest standing position with raytrace line-of-sight to the table.
            # Every standable candidate is collected first using local block lookups, then line
            # of sight for the whole set is resolved in one batched round trip. Checking them one
            # at a time costs a game tick each, which for a full scan is tens of seconds.
            bot_pos = bot.position(bot_name)
            candidates = []
            ...
            reachable = world.can_reach_blocks(
                [world.ReachQuery(x, y, z, from_pos=(cx, cy, cz)) for (_, cx, cy, cz) in candidates],
                bot_name=bot_name)

            # Nearest ring wins; within that ring, the candidate closest to the bot wins.
            stand_pos = None
            best_key = None
            for (h_dist, cx, cy, cz), ok in zip(candidates, reachable):
                if not ok:
                    continue
                dist = ((cx + 0.5 - bot_pos['x'])**2 +
                        (cy - bot_pos['y'])**2 +
                        (cz + 0.5 - bot_pos['z'])**2)
                key = (h_dist, dist)
            ...
            if stand_pos:
                ...
                baritone.goto(x, y, z, bot_name=bot_name)

            # Wait for pathfinding to start
            start_time = time.time()
            pathing_started = False
            while time.time() - start_time < 5:
                status = baritone.get_process_status(bot_name)
        ...
        world.interact_block(x, y, z, bot_name=bot_name)
...
def get_missing_materials(item_id, count=1, bot_name="", recipe_id=None):
    ...
    if recipe_id:
        ...
        recipes = world.get_recipes_for(item_id, bot_name=bot_name)
    ...
    for ingredient in recipe.get('ingredients', []):
        if ingredient.get('items'):
            for item in ingredient['items']:
                info = world.get_item_info(item, bot_name=bot_name)
                if info:
    ...
    if items_in_grid:
        for item in items_in_grid:
            world.click_slot(item['slot'], button=world.MouseButton.LEFT,
                           click_type=world.ClickType.QUICK_MOVE, bot_name=bot_name, silent=True)
        ...
        for ingredient in recipe.get('ingredients', []):
            ...
            while current_in_slot < needed_for_slot:
                ...
                world.click_slot(item_slot, button=world.MouseButton.LEFT,
                               click_type=world.ClickType.PICKUP, bot_name=bot_name, silent=True)
                time.sleep(0.05)
...
def _consolidate_stacks(item_id, bot_name=""):
    ...
    info = world.get_item_info(item_id, bot_name=bot_name)
...
def auto_craft_recursive(item_id, count=1, bot_name="", max_distance=128):
    ...
    plan = world.plan_recursive_craft(item_id, count, bot_name=bot_name)
